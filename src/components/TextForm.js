...
export default function TextForm({showAlert, mode, Heading }) {
  ...
  return (
    ...
    <div className='container' style = {{color: mode === 'dark'?'white':'#042743'}}>
        <h1 mb-2="true" disabled={text.length===0} >{Heading}</h1>
      <div className="mb-3">
        <textarea className="text form-control" value={text} 
        onChange={handleOnChange} style={{backgroundColor : mode ==='dark'?'#13466e':'white',color: mode === 'dark'?'white':'#042743'}} id="exampleFormControlTextarea1" rows="8"></textarea>
      </div>
